...
class AppConfig(BaseModel):
    ...
    port: int = 8000
    reload: bool = True
    # Замер ДЗ 6: один процесс держит 900-990 запросов/с на кэшированной ручке
    # (46 % ядра, база и Redis простаивают), четыре дают 2200. Больше воркеров на
    # локальной машине смысла не имеет — ядра нужны PostgreSQL, Redis и Kafka.
    # Учитывается только при выключенном reload: с автоперезагрузкой uvicorn
